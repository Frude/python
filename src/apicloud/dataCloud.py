...
##############################		
	...
	def createUser(self, attr):
		...
		if 'username' not in attr or not attr['username']:
			...
			return {'status':0,'msg':'user name can not be None'}
		
		if 'password' not in attr or not attr['password']:
			# return {'status':0,'msg':'密码不能为空'}
			return {'status':0,'msg':'pass cannot be None'}
	...
	def userLogout(self, loginId):
		...
		if not loginId:
			...
			return {'status':0,'msg':'user id can not be None'}
...
# 		#url += str(filter)#.decode("utf-8");
		url ='''https://d.apicloud.com/mcm/api/'''+object+'''?filter='''
